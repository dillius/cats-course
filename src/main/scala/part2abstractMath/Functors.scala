...
object Functors {
  ...
  val incrementedTry = tryFunctor.map(Try(42))(_ + 1)

  // generalizing an API with functor
  def do10xList(list: List[Int]): List[Int] = list.map(_ * 10)
  def do10xOption(option: Option[Int]): Option[Int] = option.map(_ * 10)
  def do10xTry(tryValue: Try[Int]): Try[Int] = tryValue.map(_ * 10)
  ...
  trait Tree[+T]
  object Tree {
    // "smart" constructors returning the type of the parent for implicit behavior to trigger off of
  ...
  implicit object TreeFunctor extends Functor[Tree] {
    // Not tail recursive, not dealing with that at this time
  ...
  val tree: Tree[Int] = Tree.branch(4, Tree.leaf(2), Tree.leaf(6))
  val incrementedTree = tree.map(_ + 1)

  // TODO 2 - write a shorter version of do10x by using extension methods
  def do10xv2[F[_]: Functor](container: F[Int]): F[Int] =
    container.map(_ * 10)
  ...
  def main(args: Array[String]): Unit = {
    ...
    println(do10x(Try(3)))
    println(do10x[Tree](Branch(2, Leaf(1), Leaf(3))))
    println(do10x(Tree.branch(2, Tree.leaf(1), Tree.leaf(3))))
    println(do10xv2(Tree.branch(2, Tree.leaf(1), Tree.leaf(3))))
  }
